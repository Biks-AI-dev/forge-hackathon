...
4. **Forge + hand over: THE HALO MOMENT (this is the demo's wow, spec it exactly).** Daytona sandbox, chat URL - and the link is sent TO THE PROSPECT. The prospect opens it cold and types "halo". The agent's reply must show it ALREADY KNOWS THEM: greets by name/company, names the painpoint from the meeting ("aku tahu tiap pagi Mbak Sari habis 2-3 jam cocokin mutasi..."), then delivers a personalized panduan in the EXACT structure of production baba's real guide (see the transcript pattern): (a) who I am + my job in one line, (b) "cara pakai" per ROLE with their real roles (SPG kirim closing, admin kirim mutasi japri, etc.), (c) the confirm ritual - "aku echo angkanya -> kamu balas ya -> tercatat", (d) the status legend green/amber/red, (e) an invitation to try now. Then it works: takes the closing, reads the file, computes, confirms. An AI employee that is useful and GUIDES the person from message one. Generic greetings fail acceptance.
...
- [ ] **The halo test**: a fresh browser opens the chat URL and types only "halo" - the reply greets by name/company, names the painpoint from the meeting unprompted, and delivers the role-by-role panduan in baba's structure (tugasku / cara pakai per role / echo->ya->tercatat / status legend / ajak coba). A greeting that could belong to any bot fails
